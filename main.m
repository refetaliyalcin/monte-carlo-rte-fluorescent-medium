...
cos_teta_prime=zeros(length(lamda),1); %the cos of the ray after refracted from air to medium
sur_reflection=zeros(length(lamda),1); %surface reflection at air-medium interfece

for i=1:length(lamda)
    cos_teta_prime(i)=cos(F_fresnel_2(n_medium(i),k_medium(i),polar_angle_rad));
